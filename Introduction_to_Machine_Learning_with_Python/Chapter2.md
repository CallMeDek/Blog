...
일반화 - 이전에 본 적 없는 새로운 데이터 잘 맞는 모델을 학습 시키는 것.

과소 적합 - 훈련 데이터의 다양한 특징을 반영하지 못하는 모델의 상태

과대 적합 - 훈련 데이터에 너무 잘 맞춰져 있어서 새로운 데이터에 일반적이지 못한 모델의 상태

맣은 알고리즘에서 좋은 성능을 내려면 매개 변수를 적절히 설정하느 것이 중요하다.

어떤 알고리즘은 입력 데이터의 스케일을 어떻게 하느냐에 민감하다.

모델의 가정과 매개변수의 의미를 제대로 이해하는 것이 좋다.

- 최근접 이웃 - 작은 데이터셋일 경우, 기본 모델로서 좋고 설명하기 쉬움
- 선형 모델 - 첫 번째로 시도할 알고리즘. 대용량 데이터셋 가능. 고차원 데이터에 가능.
- 나이브 베이즈 - 분류만 가능. 선형 모델보다 빠름. 대용량 데이터셋과 고차우너 데이터에 가능. 선형 모델보다 덜 정확함.
...
- 그래디언트 부스팅 결정 트리 - 랜덤 포레스트보다 조금 더 성능이 좋음. 랜덤 포레스트보다 학습은 느리나 예측은 빠르고 메모리를 조금 사용. 랜덤 포레스트보다 매개변수 튜닝이 많이 필요.
...
새로운 데이터셋을 작업할 때는 선형 모델이나 나이브 베이즈 또는 최근접 이웃 분류기 같은 간단한  모델로 시작해서 성능이 얼마나 나오는지 가늠해 보는 것이 좋다. 데이터를 충분히 이해한 뒤에 랜덤 포레스트나 그래디언트 부스팅 결정 트리, SVM, 신경망 같은 복잡한 모델을 만들 수 있는 알고리즘을 고려한다.
