...
Cascade learning은 단계적인 방법으로 강력한 분류기를 구축하기 위해서 주어진 분류기들의 출력에서 정보를 모으는 coarse-to-fine (Coarse-to-fine in the context of visual recognition typically refers to applying a method (or machine learned model) to a low-resolution version of the image, and then gradually increasing the resolution and propagating the results to the 'finer' image.) 학습 전략이다. 

- Cascade learning 전략은 Viola and Jones에 의해 강력한 얼굴 탐지기를 훈련 시키기 위하여 처음 사용되었다. 그들의 모델에서 경량의 탐지기가 먼저 대다수의 쉬운 음성들을 골라내고 다음 단계의 탐지기들을 훈련시키기 위해 hard 후보들을 보낸다. 
- 딥러닝 기반 탐지 알고리즘에서 Yang 등은 RPN과 지역 분류기들을 단계적인 학습 전략으로 학습 시키는 Cascade Region-proposal-network and FasT-rcnn(CRAFT)를 제안했다. CRAFTS는 표준적인 RPN을 학습시키고 대다수의 쉬운 음성들을 제거하는 두 클래스의 Fast RCNN을 학습시킨다. 제거하고 남은 샘플들은 두 개의 Fast RCNN으로 이루어져 있는 단계적 지역 분류기들을 구축하기 위해 사용된다. 
- Yang 등은 각기 다른 계층에서 다른 크기의 객체들을 분류하기 위한 계층별(layer-wise) cascade 분류기들을 도입했다. 여러 분류기들은 각기 다른 특징 맵들에 배치되어 얇은 층들의 분류기는 쉬운 음성들을 제거한다. 제거 하고 남은 샘플들은 깊은 층에서의 분류 작업을 위해 넘겨진다. 
- RefineDet과 Cascade R-CNN은 객체의 위치를 조정하는데 단계별 학습 방법들을 활용했다. 여기서 멀티 스테이지의 바운딩 박스 회귀기들을 구축하고 바운딩 박스 예측들이 각기 다른 퀄리티 측정 방법으로 훈련된 각 스테이지에서 조정되었다. 
- Cheng 등은 Faster RCNN에서의 실패 사례들을 관찰하고 객체들의 위치 추정이 좋을 지라도 몇가지 분류 에러가 존재함을 알아내었다. 그들은 이것이 특징들을 공유하고 멀티 테스크 최적화를 공동으로 하기 위한 차선의 특징 표현들 때문이라고 보았다. 그리고 그들은 Faster RCNN의 너무 큰 수용체 영역이 탐지 과정에서 너무 많은 노이즈를 추론한다고 주장했다. 그들은 기본 RCNN이 이 문제에 강인하다는 것을 확인했고 그래서 Faster RCNN과 RCNN이 서로를 보충하도록 하는 단계졀 탐지 시스템을 만들었다. 특히 초기의 예측값들의 집합은 잘 훈련된 RCNN에서 획득했고 이 예측들은 결과를 보정하기 위한 RCNN을 훈련시키기 위해 사용되었다. 



#### 4.1.5 Others

- Adversarial Learning - Adversarial learning은 생성 모델에서 큰 진보를 보여줬다. 이 방법을 적용한 가장 유명한 연구는 생성기가 식별자와 경쟁하는 구조의 Generative adversarial network(GAN)이다. 생성기는 노이즈 벡터 입력을 사용한 가짜 이미지들을 생성함으로서 데이터 분포를 모델링하려고 노력하고 이 가짜 이미지들은 식별기를 혼란스럽게 만든다. 식별기는 생성기와 경쟁하여 가짜 이미지들로부터 진짜 ㅣ이미지를 확인한다.  GAN과 그 변종들은 많은 도메인에 효과적임을 보여줬고 객체 탐지 영역에서도 관련된 애플리케이션을 만들었다. 
  - Li 등은 작은 크기의 객체 탐지를 위한 Perceptual GAN이라고 하는 새로운 프레임워크를 제안했다. 학습 가능한 생성기가 adversarial 구조를 통해 작은 크기의 객체들의 높은 해상도 특징 표현들을 학습한다. 특히 생성기는 낮은 해상도의 작은 지역 특징들을 높은 해상도의 특징으로 만드는 법을 학습하고 실제 높은 해상도의 특징들을 구별하는 역할의 식별기와 경쟁한다. 최종적으로 생성기는 작은 객체들에 대한 높은 퀄리티의 특징들을 만드는 법을 학습해낸다. 
  - Wang 등은 적대적 생성 예제에 의해서 훈련되는 A-Fast-R-CNN을 제안했다. 그들이 주장하길 어려운 샘플들이 점점 많아지기 때문에 폐색과 변형이 가미된 특징들을 자동적으로 만들어 내는 두 개의 블럭들을 도입했다고 했다. 특히, 학습된 마스크는 지역 특징들과 지역 분류기들 위에서 생성된다. 이 경우, 탐지기들은 좀 더 적대적인 예제들을 받게 되고 이를 통해 좀 더 강인해진다. 
- Training from Scratch - 근래의 객체 탐지기들은 ImageNet에서 미리 훈련된 분류 모델에 심하게 의존하고 있으나 분류과 탐지 사이의 데이터 분포와 손실 함수들의 편향은 성능에 부적절한 영향을 끼칠 수있다. 탐지 작업에서의 Finetuning이 이 문제를 완화할 수는 있으나 편향을 완전히 없앨 수는 없다. 게다가 분류 모델을 새로운 도메인에서의 탐지를 위해 Transferring 하는 것은 더 많은 도전을 야기한다(RGB에서 MRI 데이터 등) 그래서 미리 훈련된 모델에 의지하기 보다는 새롭게 밑부터 탐지기를 훈련해야할 필요가 있다. 탐지기들을 처음부터 훈련시키는 것의 어려움 중 하나는 훈련 데이터가 많은 경우 불충분해서 과적합으로 이어진다는 것이다. 이미지 분류와는 다르게 객체 탐지는 바운딩 박스 레벨의 주석들이 필요해서 많은 양의 탐지 데이터셋에 주석을 다는 일은 큰 시간과 노력을 요구한다(ImageNet에서는 1000개의 카테고리가 이미지 분류를 위해 준비되어 있으나 그 중 200개만이 탐지 주석을 가지고 있다). 여기에 객체 탐기들을 처음부터 훈련시키기 위한 몇가지 연구 작업들이 있다.
  - Shen 등은 탐지기들을 밑바닥부터 훈련시키 위한 Deeply Supervised Object Detectors(DSOD)라고 하는 프레임워크를 처음 제안했다. 그들이 말하길 Deep supervision와 densely connected 네트워크 구조는 최적화의 어려움을 크게 줄인다고 했다. 
  - DSOD에 근거하여 Shen 등은 다른 크기의 객체들을 위한 중간 계층의 Supervision intensities를 급격하게 조정하는 Gated recurrent feature pyramid를 제안했다. 그들은 공간적이고 의미적인 정보를 한 개의 예측 계층으로 축소 시키기 위한 Recurrent feature pyramid 구조를 정의했다. 그렇게 함으로서 파라미터의 숫자를 줄여 더 빠르게 수렴(Convergence)할 수 있도록 했다. 게다가 Feature pyramids에서의 Gate-control 구조는 객체들의 크기에 기초하여 각기 다른 강도의 supervision으로 조정한다. 
  - He등은 MSCOCO에서 탐지기들을 처음부터 훈련시키는 것을 어려운 것임을 입증했고 기본 탐지기들이 적어도 10K개의 주석이 달린 이미지들에서는 경쟁력 있는 성능을 보일 수 있음을 확인했다. 그들은 발견은 그 어떤 특정한 구조도 처음부터 모델을 훈련시키는데 필요하지 않음을 증명했고 이것은 그 전의 연구들을 반박하는 내용이다. 
- Knowledge Distillation - Knowledge Distillation은 Teacher-student 훈련 구조를 통해 모델들의 앙상블에서의 Knowledge를 한 개의 단일 모델로 여과하는 훈련 전략이다. 이 학습 전략은 이미지 분류에서 처음 사용되었다. 객체 탐지 영역에서는 몇가지 연구들이 탐지 성능을 높이기 위해서 이 훈련 scheme을 조사했다. 
  - Li 등은 무겁고 강력한 탐지기에 의해서 조심스럽게 최적화가 가이드되어지는 경량의 탐지를 제안했다. 이 경량의 탐지기는 무거운 탐지기에서의 Knowledge를 여과함으로서 비교할만한 탐지 정확도를 달성했고 그에 반해 추론 속도는 훨씬 빨랐다. 
  - Cheng 등은 Teacher-student 훈련 scheme을 통해 최적화되는 Faster R-CNN 기반의 탐지기를 제안했다. 여기서 한 R-CNN은 훈련 과정을 가이드 하기 위한 Teacher 네트워크로서 사용된다. 그들의 프레임워크는 전통적인 방식의 단일 모델 최적화 전략과 비교하여 탐지 정확도에서 개선점을 보여줬다.
  
  

### 4.2  Testing Stage

객체 탐지 알고리즘은 밀집된 예측을 도출해내기 때문에, 심한 중복 때문에 이런 예측들이 곧바로 평가에 사용될수는 없다. 게다가 다른 학습 전략들이 탐지 정확도를 개선시키기 위해 필요하기도 하다. 이런 전략들은 예측의 질을 높이거나 추론 속도를 가속기킨다. 



#### 4.2.1 Duplicate Removal

Non maximum suppression(NMS)은 중복된 거짓 양성 예측들을 제거하기 위한 객체 탐지의 필수적인 부분이다. 

![](./Figure/Recent_Advances_in_Deep_Learning_for_Object_Detection_21.JPG)



객체 탐지 알고리즘은 몇개의 중복된 예측들로 밀집된 예측들의 집합을 만들어낸다. SSD나 Deconvolutional Single Shot Detector(DSSD)와 같이 밀집한 후보들의 집합을 만들어내는 One-stage 탐지기 알고리즘 같은 경우, 같은 객체를 감싸고 있는 후보들은 비슷한 Confidence score들을 가지고 있을수 있기 때문에 거짓 양성이 야기될 수 있다. 밀도가 낮은 후보들을 생상하는 Two-stage 탐지기 알고리즘 같은 경우에는 바운딩 박스 회귀기가 이런 후보들을 객체에 가까이 붙여놓기 때문에 One-stage와 마찬가지의 문제를 야기한다. 중복된 예측들은 거짓 양성들로 판단되어 평가 단계에서 패널티를 받게 될것이므로 NMS는 이런 중복된 예측들을 제거할 필요가있다. 특히 어떤 카테고리에 관해서는 예측 박스들이 Confident 점수에 따라 정렬되고 가장 높은 점수의 박스가 선택된다. 이런 선택된 박스를 M이라고 표시한다. 그런 다음 M과 다른 박스들의 IoU가 계산되고 IoU 값이 미리 정의 된 threshold Ωtest 보다 클 경우, 이 박스들은 제거된다. 이런 과정은 모든 남아있는 예측들에 관해 반복된다. 좀 더 공식적으로, Ωtest 보다 크게 M과 겹쳐진 박스 B의 Confident score는 0으로 지정될 것이다. 

![](./Figure/Recent_Advances_in_Deep_Learning_for_Object_Detection_22.JPG)

하지만, 어떤 객체가 M의 Ωtest 안에 놓여져 있다면 NMS는 잘못된 예측을 할 것이다. 이런 시나리오는 클러스터화된 객체 탐지에서는 매우 흔한 경우이다. 

- Navaneeth 등은  위의 문제를 해결할 새로운 알고리즘인 Soft-NMS을 도입했다. B 예측을 단순히 제거하는 것 대신에 Soft-NMS에서는 M과의 중첩함에 대한 연속 함수 F(F는 선형 함수 혹은 가우시안 함수가 될 수 있다)로 B의 Confidence 점수를 쇠퇴(decay)시켰다. Soft-NMS는 클러스터화된 객체들의 예측을 제거하는 것을 피했다. 

![](./Figure/Recent_Advances_in_Deep_Learning_for_Object_Detection_23.JPG)

- Hosong 등은 Confidence scores와 바운딩 박스들에 기초한, NMS를 수행할 네트워크 아키텍쳐를 도입했고 이 네트워크는 탐지기 훈련과는 별개로 지도 학습 방법으로 최적화되었다. 그들이 주장하길 예측들의 중복의 이유는 탐지기가 고의적으로 한 개의 높은 점수에 보상(사례)하는 것이 아니라 객체 하나 당 여러 높은 점수의 예측을 만들어 내도록 조장하기 때문이라고 했다. 이런 주장에 근거하여 그들은 다음의 두가지 동기(motivation)을 따르는 네트워크를 설계했다. 

  - 탐지기들이 정확히 객체 당 한 개의 정확한 예측을 만들어내도록 하기 위해서 두개 이상의 탐지는 손실 페널티를 준다.
  - 탐지기에게 하나의 객체가 한 번이상 탐지되었는지의 정보를 제공하기 위해서 가까이 있는 탐지들을 공동으로 처리한다. 

  이 모델은 탐지들을 버리지는 않고 대신에 이미 탐지된 객체들을 커버하는 탐지들의 점수를 감소시키는 것을 추구하는 re-scoring 작업을 하도록 NMS를 재구성했다. 



#### 4.2.2 Model Acceleration

실전에서의 애플리케이션을 위한 객체 탐지의 적용은 효율적인 방법으로 동작하는 알고리즘을 요구한다. 그러므로 탐지기들을 평가하는 효율 측정 방법이 중요하다. 현재의 최신 기술들이 공공연하게 알려진 데이터에 대해 강력한 결과를 달성하기는 했어도 그들의 추론 속도는 이것들을 실제로 적용하기 어렵게 만든다. Two-stage 탐지기들은 보통 One-stage 탐지기들보다 느린데 그 이유는 이거들이 두 단계(하나는 후보 생성 그리고 다른 하나는 지역 분류)를 가지고 있어서 후보 생성과 지역 분류를 한꺼번에 하는 하나의 네트워크를 사용하는 One-stage 탐지기들보다 컴퓨팅적으로 시간 낭비가 심하기 때문이다.  

- R-FCN은 공간적으로 민감한 특징 맵들을 구축하고 컴퓨팅 비용을 공유하기 위해서 위치에 민감한 ROI Pooling으로 특징들을 추출한다. 그러나, 공간적으로 민감한 특징 맵들의 채널들의 숫자는 카테고리 숫자만큼 상당히 증가한다. 
- Li 등은 모든 계산을 공유하는 것 대신에 최종 특징 맵들의 숫자를 꽤 감소시키는(1024에서 16으로) Light Head R-CNN을 제안했다. 그러므로 계산이 지역들 사이에서 공유되지 않긴 해도 비용은 잊어버릴만 하다.

중추 아키텍처의 측면에서 객체 탐지에서의 주요 계산 비용은 특징 추출이다. 하나의 간단한 아이디어는 탐지 중추를 좀 더 효율적인 중추로 바꿔서 탐지 속도를 가속화 하는 것이다. 

- MobileNet은 다른 작업들에서도 도입된, 깊이별 컨볼루션 계층을 가진 효율적인 CNN모델이다. 
- PVANet은 비선형 계산을 줄이고 추론 속도를 가속화 하기 위해서 CRelu와 함께 제안된 네트워크 구조이다.

오프라인(한 번 학습되면 다시 학습되지 않는) 모델을 최적화 하기 위한 다른 접근법에는 학습된 모델에서의 모델 압축과 양자화 같은 것이 있다. 최종적으로, NVIDIA에서는 배치하기 위한 학습된 모델의 계산을 최적화 해서 추론 속도를 상당히 높이는 가속화 툴킷인 TensorRT를 릴리즈했다. 



#### 4.2.3 Others

테스트 단계에서의 다른 학습 전략은 주로 탐지 정확도를 개선시키기 위해 입력 이미지를 변형하는 것으로 이루어져 있다. 

- Image pyramid는 탐지 결과를 개선시키기 위해 널리 사용되는 기술 중 하나로 다른 크기의 계층적인 니미지 셋을 구축하고 이 모든 이미지들에서 예측을 만들어낸다. 최종 탐지 예측은 각 이미지에서의 예측들을 병합한다. 
- Zhang 등은 다른 크기의 객체들을 다루기 위해 좀 더 확장적인 Image pyramid 구조를 사용했다. 그들은 테스트 이미지를 특정한 크기 범위에 드는 객체들을 책임지도록 각기 다른 크기로 재조정했다. 
- 가로 반전도 테스트 단계에서 사용되었고 꽤 결과에서의 성능 향상을 보여줬다. 

이런 학습 전략들은 탐지기의 다른 크기의 객체들을 다룰 수 있는 능력을 개선시키고 공공 데이터 탐지 대회들에서 널리 사용되었다. 그러나 한편으로는 계산 비용을 증가시키고 그래서 실전에서 적용하기에 적합하지 않게 만들었다. 
...
- Pascal VOC2007은 20개의 카테고리를 가진 중간 규모의 객체 탐지를 위한 데이터 셋이다. 훈련 데이터 2501장, 검증 데이터 2510, 테스트 데이터 5011장이 있다. 

- Pascal VOC2012는 2007의 20개의 카테로리를 포함하는 객체 탐지를 위한 중간 규모의 데이터 셋이다. 훈련 데이터 5717, 검증 데이터 5823, 테스트 데이터 10991장이 있는데 테스트 데이터의 주석 정보는 이용 불가하다. 

- MSCOCO는 80개의 카테고리를 가진 큰 규모의 데이터셋이다. 트레이닝 118287, 검증 5000, 테스트 40670장이 있고 테스트 데이터의 주석 정보는 이용불가다. 

- Open Images는 600개의 카테고리의 15M의 객체를 포함하는 1.9M의 이미지이다. 가장 많이 나타나는 500개의 카테고리들은 탐지 성능을 평가하기위해 사용되고 이 카테고리들의 70%이상이 1000장 이상의 샘플을 가지고 있다. 

- LVIS는 1000개 이상의 카테고리의 164000 이미지를 가지고 있다. 2.2 백만의 높은 퀄리티의 Instance segmentation 마스크를 가지고 있다. 맣은 수의 카테고리가 있지만 카테고리당 데이터는 가끔 드물기도 하다. 

- ImageNet은 200개이상의 카테고리를 가지고 있으나 크기가 방대하고 객체 크기의 범위가 VOC 데이터셋과 비슷하다 그래서 보통 탐지 알고리즘에는 잘 안 쓰인다. 

- 평가 측정 방법에는 탐지 정확도와 추론 속도가 탐지 알고리즘을 평가하기 위해 사용된다. 탐지 정확도를 측정하기 위해서 mean Average Precision(mAP)가 모든 대회에서 측정 방법으로 사용된다. VOC2012, VOC2007과 ImageNet에서 mAP의 IoU threshold는 0.5이다. MSCOCO에서는 좀더 포괄적인 평가 측정 방법이 적용된다. 각기 다른 IoU threshold와 다른 크기의 객체들을에서의 성능을 포함하는, 탐지 알고리즘의 각기 다른 능력을 입증하는 6개의 평가 점수가 있다. 

  ![](./Figure/Recent_Advances_in_Deep_Learning_for_Object_Detection_24.JPG)



## 7. State-of-the-art for Generic Object Detection

Pascal VOC2007, VOC2007, MSCOCO는 탐지 알고리즘을 평가하는데 가장 흔하게 사용되는 데이터셋이다. Pascal VOC2012, VOC2007은 이미지당 2개 혹은 3개의 객체들이 있는 중간 규모의 데이터셋이고 VOC 데이터셋의 객체 크기의 범위는 크지 않다. MSCOCO는 이미지당 거의 10개의 객체들이 있고 주요 객체들은 넓은 크기 범위의 작은 객체들이고 이는 객체 알고리즘에게 큰 도전 과제이다.
